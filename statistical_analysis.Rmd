...
fit_sigD_live_cells <- data_within_sigD %>% 
  glm(cbind(`live cells`, `dying cells` + `live cells`) ~ time, family = "binomial", data = .)
...
Finally, we can look at the difference in percentage of NTs between live and dead cells at t = 90. Similarly to previous fits where one condition is all zeroes, we add 1 to all counts to regularize.
...
The proportion of live cells decreases significantly at t = 90 (p < 0.01).
...
We replace 0 transformations with 1s to avoid taking the log of 0 - showing only the DNAse- conditions.
...
```{r}
data_S8_12 %>% group_by(category, condition, time) %>%
...
```{r}
for(condition_to_test in setdiff(unique(data_s8_12_model$condition), "Empty LB")) {
  cat("\n================================\n\n Condition :", condition_to_test, "\n================================\n\n")
...
We see that the Cm and Rif fits are problematic due to large number of zeroes, so we reinvestigate the Cm condition with regularization by adding 1 to all cells and with RStanArm.

First with adding 1:

```{r}
fit_cm_time_NT_1 <- data_s8_12_model %>%
  filter(condition == "Cm5") %>%
  glm(cbind(total_NTs + 1, total_cells - total_NTs + 1) ~ time_num, family = "binomial", data = .)

summary(fit_cm_time_NT_1)
```

Then the same with RStanArm and weakly regularizing priors:

```{r, message =FALSE, results="hide"}
fit_cm_time_NT_stan <-  data_s8_12_model %>%
  filter(condition == "Cm5") %>%
...
```{r}
summary(fit_cm_time_NT_stan, probs = c(0.005,0.025, 0.975,0.995))
```


Now Rif First with adding 1:

```{r}
fit_Rif_time_NT_1 <- data_s8_12_model %>%
  filter(condition == "Rif") %>%
  glm(cbind(total_NTs + 1, total_cells - total_NTs + 1) ~ time_num, family = "binomial", data = .)

summary(fit_Rif_time_NT_1)
```

Then the same with RStanArm and weakly regularizing priors:

```{r, message =FALSE, results="hide"}
fit_rif_time_NT_stan <-  data_s8_12_model %>%
  filter(condition == "Rif") %>%
  rstanarm::stan_glm(cbind(total_NTs, total_cells - total_NTs) ~ time_num, family = "binomial", data = ., 
                   prior_intercept = normal(0,10), prior = normal(0,5),
                   warmup = 1000, iter = 6000) # Large number of iterations to accurately sample tails of the distributions
...
```{r}
summary(fit_rif_time_NT_stan, probs = c(0.005,0.025, 0.975,0.995))
```

Both regularized fits for Cm5 and Rif don't show significant change, but also cannot rule out non-negligible effect.
...
All the contrasts of `Spc, Cm` in the baseline (dnase-) condition against everything are significant. 
