...
output:
  ...
  pdf_document:
    toc: yes
abstract: |+
  The statistical analysis is presented here in the same order as the data/figures in the main paper. All code can be accessed from this document or at https://github.com/cas-bioinf/nanotubes-death.
---
...
# Figure S1 - NTs under SEM in varying genetic background
...
Once again, no differences can be deemed significant - however, the data also cannot rule out that the true differences are substantial as witnessed by the wide posterior credible intervals. Importantly, as seen in Fig. 5 - when a different microscopic technique was used - the effect of the absence of $sigD$ on formation of NTs was significant.
...
```{r}
wt_sigD_data %>% group_by(condition) %>% summarise(ratio_NT_all = sum(total_NTs) / sum(total_cells), ratio_NT_dying = sum(`dying cells NTs`) / sum(`dying cells`))
```
...
Now let's look at the proportion of NTs in dying cells only, _i. e._ dying cells with NTs are treated as success and dying cells without NTs are treated as failures:
...
```{r}
wt_lytEF_data %>% group_by(condition) %>% summarise(ratio_NT_all = sum(total_NTs) / sum(total_cells), ratio_NT_dying = sum(`dying cells NTs`) / sum(`dying cells`))
```
...
NTs are formed at significantly lower rate in $\Delta lytEF$ (p < 0.001).

Now let's look at the proportion of NTs in dying cells only:
...
First, let's look at the number of dying cells for each condition (live cells treated as success and dying cells as failures):
...
Now let's look at the number of total NTs. We test only the Amp500 condition, because other conditions have at most one NT over all fields scanned.
