...
Similarly, to the paper, we will abbreviate nanotubes as NTs. In each section, we show the relevant data either as a table or in a plot and then follow to perform the statistical analysis.
...
We will take advantage of the fact that given a uniform prior distribution for the probability of a binary outcome, the  posterior distribution after observing $n$ successes and $k$ failures is $Beta(n + 1, k + 1)$. This also coincides with a frequentist confidence interval.
...
Is the the frequency of NT formation in $\Delta sigD$ (at 90 min)  lower than in wt (at 25 min)? (both at P-GLG). Let's look at the ratio of cells with NTs among all cells and among dead cells:
...
First, let's look at the proportion of NTs across all cells, ie. cells with NTs are treated as succes while cells without NTs are treated as failures in a binomial generalized linear model:
...
## Do Ampicillin treated cells produce less NTs than p-GLG? Lets look at total NTs first:

Similarly to Figure 2, we will use a binomial generalized linear model. Cells with NTs are treated as succes and cells without NTs as failures.
