---
title: "Bacterial nanotubes are a manifestation of cell death: Statistical Analysis"
output:
  html_document:
    code_folding: hide
    toc: yes
...
#    toc: yes
abstract: |+
  This documents contains details of the statistical analysis for the main paper. The analysis is presented here in the same order as the data/figures in the main paper. Code to reproduce the analysis can be found at https://github.com/cas-bioinf/nanotubes-death.
---
...
```{r}
read_single_sem_quantification <- function(sheet, replicate,  range, condition, space = FALSE) {
  if(space) {
    col_types = c("numeric","skip", "numeric")
  } else {
    col_types = c("numeric","numeric")
  }
  read_excel(source_file, sheet, range = range, col_names = c("Cells","NTs"), col_types = col_types) %>%
    mutate(condition = condition, replicate = replicate, field = 1:n())
}

read_condition_sem_quantification <- function(sheet, cols, rows_from, rows_to, condition, space = FALSE) {
  res <- list()
  for(i in 1:length(rows_from)) {
    res[[i]] <- read_single_sem_quantification(sheet = sheet, replicate = paste0("E",i), range = 
...
s1lsheet <- "Fig.S1 chart l"

data_S1l <- rbind(
  read_condition_sem_quantification(s1lsheet, c("AS","AT"), rows_from = c(2,7,9,11), rows_to = c(4,7,9,12), condition = "-SigD"),
  read_condition_sem_quantification(s1lsheet, c("AB","AC"), rows_from = c(2,7,11,16), rows_to = c(4,8,13,17), condition = "-SigI"),
  read_condition_sem_quantification(s1lsheet, c("S","T"), rows_from = c(2,7,14), rows_to = c(4,10,14), condition = "-7ECF"),
  read_condition_sem_quantification(s1lsheet, c("AK","AL"), rows_from = c(2,7), rows_to = c(4,9), condition = "-SigB"),
  read_condition_sem_quantification(s1lsheet, c("J","K"), rows_from = c(2,8,14), rows_to = c(4,11,15), condition = "-hag")
)  
...
```{r}
read_cell_count_data <- function(sheet, prefix) {
  readxl::read_excel(source_file, sheet = sheet, range = "F1:V4", 
...
glg <- read_cell_count_data("Fig.2 chart b", prefix = "Wt ") %>% mutate(condition = "GLG")
p_glg <- read_cell_count_data("Fig.2 chart c", prefix = "Wt ") %>% mutate(condition = "P-GLG")
...
```{r}
data_S6ab <- read_cell_count_data(sheet = "Fig S6 chart ab", prefix = "SigD ") %>%
...
data_S6de <- read_cell_count_data(sheet = "Fig S6 chart de", prefix = "LytEF ") %>% mutate(condition = "-lytEF P-GLG")
...
```{r}
read_dnase_subset <- function(sheet, range, condition) {
    readxl::read_excel(source_file, sheet = sheet, range = range,
...
read_fig4_subset <- function(range, condition) {
  read_dnase_subset(sheet = "Fig.4 chart b", range = range, condition = condition)
}
...
```{r}
data_fig4c <- readxl::read_excel(source_file, sheet = "Fig.4 chart c", range = "C2:G4",
...
read_single_receptive_field <- function(sheet, row, column) {
  ...
  title_data <- readxl::read_excel(source_file, sheet = sheet, range = range_title, col_names = "data")
  ...
  readxl::read_excel(source_file, sheet = sheet, range = main_range, col_names = c("category", "modifier", paste0("time", c(0, 30, 60, 90))),
...
data_S8_12 <- rbind(
  read_all_receptive_fields("Fig.S8 charts bc", c(3,3,3)),
  read_all_receptive_fields("Fig.S9 charts bc", c(5,5,5)),
  read_all_receptive_fields("Fig.S11 charts bc", c(2,2,2)),
  read_all_receptive_fields("Fig.S12 charts bc", c(3,2,2))
)  %>%
...
read_figS14_subset <- function(range, condition) {
  read_dnase_subset(sheet = "Fig.S14 chart c", range = range, condition = condition)
}
...
```{r}
data_S16d <- rbind(
  read_condition_sem_quantification(sheet = "Fig.S16 chart d", c("S","T"), condition = "SEM", rows_from = c(2,8,14), rows_to = c(4,11,15)),
  read_condition_sem_quantification(sheet = "Fig.S16 chart d", c("G","H"), condition = "SIM", rows_from = c(2,10,15), rows_to = c(7,11,21))
) %>%
...
```{r}
data_S16h <- rbind(
  read_condition_sem_quantification(sheet = "Fig.S16 chart h", c("W","Y"), condition = "SEM", rows_from = c(3,12,20), rows_to = c(7,15,26), space = TRUE),
  read_condition_sem_quantification(sheet = "Fig.S16 chart h", c("J","L"), condition = "SIM", rows_from = c(4,7,10), rows_to = c(4,7,10), space = TRUE)
) %>%
  mutate(condition = factor(condition, levels = c("SIM","SEM")))
