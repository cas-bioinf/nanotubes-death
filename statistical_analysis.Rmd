...
redo_plots <- FALSE # Set to TRUE to recreate variants of all the figures (originally used to check that the data was loaded correctly)
...
# Figure S1l - NTs under SEM in varying genetic background
...
data_S1l <- rbind(
  ...
  read_condition_sem_quantification("SEM", c("J","K"), condition = "-hag", rows_from = c(2,8,14), rows_to = c(4,11,15), file = "S17d Quantification of NTs in liquid medium.xlsx")
)  
...
To compare the conditions, we will use a binomial generalized linear model. Here, we will treat `condition` (the individual knock-outs performed) as the sole covariate. Cells with NT are treated as success, cells without NTs as failures. Since the $\Delta sigD$ condition has no NTs at all we cannot use the model directly, instead we will try regularizing by adding one success and one failure to each condition and by fitting a Bayesian GLM with weakly informative priors. First the frequentist fit with regularization:
...
Once again, no differences can be deemed significant - however, the data also cannot rule out that the true differences are substantial as witnessed by the wide posterior credible intervals.
...
if(redo_plots) {
  ...
  ## Fig 2b
  print(plot_cell_count_data_alive(fig2_data))
  ...
  ## Fig 2c
  print(plot_cell_count_data_NTs(fig2_data))
}
```
...
Here, we treat dead cells with NTs as success and dead cells without NTs as failure.
...
There is good evidence that there are more dead cells and more cells with NTs  in the P-GLG condition (p < 0.001 and p = 0.04 respectively), but it is not clear whether a single dead cell is more likely to have NTs in the P-GLG condition.

# Figure S5 a,b - Frequency of NT formation in $\Delta sigD$
...
if(redo_plots) {
  plot5a <- plot_cell_count_data_alive(data_S5ab, NULL) + expand_limits(y = 1)
  plot5b <- plot_cell_count_data_NTs(data_S5ab, facet_wrap(~alive_string)) + expand_limits(y = 0.1)
  plot5a + plot5b
}
```


Is the the frequency of NT formation in $\Delta sigD$ (at 90 min)  lower than in wt (at 25 min)? (both at P-GLG). Let us look at the ratio of cells with NTs among all cells and among dead cells:
...
First, we examine the proportion of NTs across all cells, ie. cells with NTs are treated as succes while cells without NTs are treated as failures in a binomial generalized linear model:
...
Here, we get significantly lower rate of overall NT formation in $\Delta sigD$ (p < 0.001).

Now let's look at the proportion of NTs in dead cells only, i.e. dead cells with NTs are treated as success and dead cells without NTs are treated as failures:
...
If we look at the dead cells only, we also have significantly lower rate of NT formation in $\Delta sigD$. To summarise, the frequency of NT formation in $\Delta sigD$ (at 90 min) appeared to be lower than in wt P-GLG (at 25 min) (all p < 0.001).
...
There are no NTs from live cells, but we can look at NTs from dead cells. 

```{r}
fit_sigD_NTs_dead <- data_within_sigD %>%  
  glm(cbind(`dying cells NTs`, `dying cells` - `dying cells NTs`) ~ time, family = "binomial", data = .)

summary(fit_sigD_NTs_dead)
```

The proportion of NTs from dying cells at t=0 is lower than the proportion at t = 90, but the difference is not significant.


Finally, we can look at the difference in percentage of NTs between live and dead cells only at t = 90. Similarly to previous fits where one condition is all zeroes, we add 1 to all counts to regularize.


```{r}
fit_sigD_NTs <- data_S5ab %>%
  filter(time == 90) %>%
...
With this regularization we can say that dying cells produced signicantly more NTs than living cells (p = 0.03).

# Figure S5 d,e - Frequency of NT formation in $\Delta lytEF$

Now we will do the same analysis we did for $\Delta sigD$, but using data for $\Delta lytEF$. 
...
#Recreate figures S5d,e to check if the data were loaded correctly.
if(redo_plots) {
  figS5d <- plot_cell_count_data_alive(data_S5de, NULL) + expand_limits(y = 1)
  figS5e <- plot_cell_count_data_NTs(data_S5de, facet_wrap(~alive_string)) 
  figS5d + figS5e
}
```
...
Significantly lower rate in $\Delta lytEF$ (p < 0.001).
...
If we look at the dead cells only, we also have significantly lower rate in $\Delta lytEF$. To summarise, the frequency of NT formation in $\Delta lytEF$ (at 90 min) appeared to be lower than in wt P-GLG (at 25 min) (all p < 0.001).
...
There are very few NTs overall, and many conditions contain only zero, so we will once again add 1 to all conditions for regularization. First we look at NTs from dead cells. 

```{r}
fit_lytEF_NTs_dead <- data_within_lytEF %>%  
  mutate(`dying cells NTs` = `dying cells NTs` + 1) %>%
  glm(cbind(`dying cells NTs`, `dying cells`) ~ time, family = "binomial", data = .)

summary(fit_lytEF_NTs_dead)
```

The proportion of NTs from dying cells at t=0 is not significantly different from the proportion at t = 90.


Finally, we can look at the difference in percentage of NTs between live and dead cells at t = 90. 


```{r}
fit_lytEF_NTs <- data_S5de %>%
  filter(time == 90) %>%
  select(-alive_string, -time, -type) %>%
  mutate(category = str_remove(as.character(category), "live |dying ")) %>%
  mutate(count = count + 1) %>%
  pivot_wider(names_from = category, values_from = count) %>%
  glm(cbind(cells, `cells NTs`) ~ alive, data = ., family = "binomial")
...
if(redo_plots) {
  fig4_data %>% filter(dnase == "baseline")  %>% ggplot(aes(x = condition, y = ratio_100)) + geom_point(color = "gray", position = position_jitter(width = 0.3)) + stat_summary(fun.data = mean_se) + scale_y_log10()
}
...
We will model the logarithm of the ratio of transformations with a normal linear model with condition and DNAse presence/absence as covariates (including their interaction).
...
We start by examining the correlation visually, replacing 0s in the transformation data for comK with 1/number of recipients to avoid taking the log of 0.
...
Then we run Pearson's correlation test on the data:
...
# Figure S8-S12 - Antibiotic treatments
...
Let us look at total NTs first. Similarly to Figure 2, we will use a binomial generalized linear model. Cells with NTs are treated as succes and cells without NTs as failures.
...
Treatment with Ampicilin led to death of majority of the cells and induced NT formation from dead cells although less frequently than the P-GLG method (p < 0.001). "


## Do number of dead cells change over time?

Here, we treat time as continous, i.e. we test the overall _trend_ over time, not the difference between individual time points, technically we treat time as a continous covariate in the model.
...
```{r}
summary(fit_s8_12_time_dead_amp_stan, probs = c(0.005,0.025, 0.975,0.995))
```


We see significant changes in number of dead cells over time for all but the EmptyLB (no antibiotic) condition (all p < 0.001). The EmptyLB condition is consistent with no change, but we also cannot rule out some noticeable changes.

## Do number of total NTs change over time

Now let's look at the number of total NTs. We test only the Amp500 condition, because other have at most one NT over all fields scanned.
...
The time trend of the number of dead cells is statistically significant for all antibiotic conditions (all p < 0.001). Time trend of the number of NTs is statistically significant only for Ampicilin (p < 0.001).

# Figure S8 - P-GLG vs. Ampicilin

We further compare the proportion of NTs from dead cells in the P-GLG condition (data from Figure 2) and the Ampicilin condition.
...
data_amp_30 <- data_s8_12_model %>% filter(condition == "Amp500", time == 30) %>% 
  mutate(replicate = paste0(replicate, "-", field)) %>% select(-field, -time_num)

fit_pglg_amp <- glm(cbind(`dying cells NTs`, `dying cells`) ~ condition, family = "binomial", data = rbind(data_p_GLG_25, data_amp_30))
...
# Figure S10 - Spatial distribution of NTs after Ampicilin treatment
...
if(redo_plots) {
  ...
  figS14_data %>% filter(dnase == "baseline")  %>% ggplot(aes(x = condition, y = ratio_100)) + geom_point(color = "gray", position = position_jitter(width = 0.3)) + stat_summary(fun.data = mean_se) + scale_y_log10()
}
...
```{r}
data_S17d <- rbind(
  read_condition_sem_quantification("SEM", c("J","K"), condition = "SEM", rows_from = c(2,8,14), rows_to = c(4,11,15), file = "S17d Quantification of NTs in liquid medium.xlsx"),
  read_condition_sem_quantification("SIM", c("G","H"), condition = "SIM", rows_from = c(2,10,15), rows_to = c(7,11,21), file = "S17d Quantification of NTs in liquid medium.xlsx")
)

if(redo_plots) {
  data_S17d %>% group_by(condition, replicate) %>%
    summarise(prop_NTs = sum(NTs) / sum(Cells), .groups = "drop") %>%
    group_by(condition) %>%
    summarise(petc_NTs_mean = mean(prop_NTs) * 100, perc_NTs_SEM = sd(prop_NTs) / sqrt(n()) * 100, .groups = "drop")
}
...
```{r}
fit_S17d <- glm(cbind(NTs, Cells - NTs) ~ condition, data = data_S17d, family = "binomial")
summary(fit_S17d)
...
```{r}
data_S17h <- rbind(
  read_condition_sem_quantification("SEM", c("J","L"), condition = "SEM", rows_from = c(3,12,20), rows_to = c(7,15,26), file = "S17h Quantification of NTs on solid medium.xlsx", space = TRUE),
  read_condition_sem_quantification("SIM", c("J","L"), condition = "SIM", rows_from = c(4,7,10), rows_to = c(4,7,10), file = "S17h Quantification of NTs on solid medium.xlsx", space = TRUE)
)

if(redo_plots) {
  data_S17h %>% group_by(condition, replicate) %>%
    summarise(prop_NTs = sum(NTs) / sum(Cells), .groups = "drop") %>%
    group_by(condition) %>%
    summarise(petc_NTs_mean = mean(prop_NTs) * 100, perc_NTs_SEM = sd(prop_NTs) / sqrt(n()) * 100, .groups = "drop")
}
...
```{r}
fit_S17h_reg <- glm(cbind(NTs + 1, Cells - NTs + 1) ~ condition, data = data_S17h, family = "binomial")
summary(fit_S17h_reg)
...
```{r, message=FALSE, results = "hide", cache = TRUE}
fit_S17h_stan <- 
  rstanarm::stan_glm(cbind(NTs, Cells - NTs) ~ condition, family = "binomial", data = data_S17h, 
                     prior_intercept = normal(0,10), prior = normal(0,5),
                     warmup = 1000, iter = 6000) # Large number of iterations to accurately sample tails of the distributions
```

```{r}
summary(fit_S17h_stan, probs = c(0.0005, 0.025, 0.975, 0.9995))
